...
test('deterministic infrastructure manifest validates', async () => {
  ...
  const manifest = {
    ...
    task: 'infrastructure' as const,
    deterministic: true,
    hypothesis: 'Testing as const infrastructure checks run without model',
...
test('model-based parse manifest validates', async () => {
  ...
  const manifest = {
    schema: 'openlunum-experiment/0.1' as const,
    id: 'test-parse',
    area: 'multilingual-parse' as const,
    task: 'parse' as const,
    hypothesis: 'Testing as const parse task with model profile',
    baselineCommit: 'ca623ec',
