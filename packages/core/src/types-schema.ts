...
export interface ProtectedDatasetSchema {
  schema: "openlunum-protected-dataset/0.1";
  id: string;
  path: string;
  sha256: string;
  version: string;
  items?: number;
  languages?: string[];
  access: "evaluator-only" | "maintainer-only" | "review-on-demand";
  evaluatorInstructionsPath: string;
  license: string;
  created?: string;
  provenance?: string;
  warning?: string;
