...
- [x] Add a tokenizer-optimization pass: for each named local model in Token Atlas, produce a model-specific tight profile that provably does not change semantics. *(Current main sets `optimizedFingerprint = entry.fingerprint` and compares the value to itself, so preservation is tautological. Draft repair: PR #184.)*
