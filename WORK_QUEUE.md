...
Most v4 implementation work has landed, but the queue is **not mechanically complete**. A maintainer audit on 2026-07-19 identified four claims whose current evidence or execution does not satisfy the stated acceptance text. Do not report v4 as 100% accepted until the unchecked items below are repaired, verified on an exact hosted-CI head, and independently reviewed.
...
- [ ] Quality gate CI integration: run quality gates on every PR that touches `packages/core/src/` or `packages/eval/src/`. *(Current main masks the gate with `|| true` and uses an obsolete fallback record. The outage repair bundle adds a tested runner, valid fallback construction, correct warning/failure exit semantics, workflow summaries, and fail-closed execution. Keep unchecked until hosted Actions executes the exact head.)*
