...
| Controlled internal pilot | **91%** | Core paths usable with narrow domains and mandatory natural-language fallback; expanded datasets, contracts, threat model, retrieval infrastructure, hard invariant gates, uncertainty policy, audit-trailed rollback, product-level safety review policy, agent retention policies, profile quality measurement, platform support matrix, release governance, downstream integration contracts, cross-framework agent interoperability, red-team review infrastructure, multilingual memory pilot simulation, security self-assessment, parse threshold validation with regression detection, retention pass runner with drift detection, compaction execution runner and crash recovery simulation in place | Requires monitoring, explicit supported inputs, rollback and human review for consequential use |
| General production dependency | **71%** | Substantial foundations including versioned contracts, auth, rate limiting, streaming, agent-state freeze, tamper evidence, idempotency, prohibited domains, structured observability, workflow audit, incident response, fingerprint contract, package governance, upgrade compatibility, downstream integrations, independent scorer evaluation, external replication, user correction telemetry, security self-assessment, crash/disk-pressure recovery simulation and compaction execution infrastructure; adoption and live evidence gaps remain | Not suitable as an unattended universal parser, safety authority or guaranteed context compressor |
...
### R12 — HTTP API, MCP and adapters: 90% → 100%
