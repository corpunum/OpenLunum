...
### Added — Safety (cfee310, PR #154)
- **Rollback process:** `rollbackToSource()` and `rollbackBatch()` verify integrity, provenance, and source authenticity before reverting Lunum-Sem records to original natural-language text. Separate integrity/provenance/source-authenticity statuses (verified/failed/absent). Fail closed when evidence is absent. `verifySourceAuthentic()` checks source text against external digests. Types in `packages/core/src/rollback-process.ts`. 10 unit tests in `packages/core/test/rollback-process.test.ts`.
