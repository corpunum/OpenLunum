...
### Added — Live-Evidence Repairs (PR #220 draft, commit 235be01)
- **WORK_QUEUE v5 — live-evidence repairs:** The 2026-07-20 live test campaign revealed that the parse runner discarded its own prompt, so every historical parse/retention result scored garbage. WORK_QUEUE v5 documents the repair path: (1) fix `parse-experiment` CLI arg handling so `node cli.js parse-experiment <manifest>` works, (2) fix the parse runner to send `parsePrompt(item).system` instead of the generic experiment-runner string, (3) add `max_tokens` (default 4096, profile-overridable) to `OpenAICompatibleModel.complete` for thinking models, (4) embed the Lunum-Sem schema shape plus one canonical one-shot example in `parsePrompt`, (5) ship a controlled predicate/role vocabulary with the parse prompt, (6) score near-semantic fingerprint matches alongside exact matches in parse experiments, (7) re-run the 4-language (EN/EL/ES/ID) parse+retention experiment against at least 2 named local models AFTER the P0/P1 fixes, and (8) recalibrate parse/retention gate thresholds from the honest baselines. (commit 235be01)
