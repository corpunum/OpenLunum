...
- Merge policy: `scripts/pi-merge-policy.mjs` with fail-closed exact-head enforcement. Required checks: verify, schema-drift, report-validation, protected-data-boundary; quality-gates for core/eval source changes. Blocks drafts, conflicts, blocking labels (needs-work, needs-rebase, maintainer-blocked), unresolved current-head NEEDS_WORK, stale approvals, missing/non-successful exact-head checks, and checks with zero recorded steps. Uses `--match-head-commit` on merge. CI_OUTAGE flag (`reports/orchestrator/CI_OUTAGE`) skips hosted checks during GitHub Actions billing outages. 14 policy tests in `scripts/pi-merge-policy.test.mjs`. (PR #190)
...
| Merge policy | Reference implementation | `scripts/pi-merge-policy.mjs` with fail-closed exact-head enforcement. Required checks: verify, schema-drift, report-validation, protected-data-boundary; quality-gates for core/eval. Blocks drafts, conflicts, blocking labels, unresolved current-head NEEDS_WORK, stale approvals, missing/non-successful exact-head checks, zero-step checks. Uses `--match-head-commit`. CI_OUTAGE flag for billing outages. 14 policy tests in `scripts/pi-merge-policy.test.mjs`. |
