...
- Rollback process: `rollbackToSource()` and `rollbackBatch()` verify integrity, provenance, and source authenticity before reverting Lunum-Sem records to original natural-language text; `verifySourceAuthentic()` checks against external digests.
...
| Rollback process | Prototype | `rollbackToSource()` and `rollbackBatch()` with integrity/provenance/source verification; separate status levels (verified/failed/absent); fail-closed on absent evidence; 10 unit tests.
