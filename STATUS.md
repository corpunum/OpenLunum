...
- Merge control: `scripts/pi-merge-policy.mjs` (194 lines) with `evaluateMergePolicy()` implementing fail-closed exact-head policy — gates on drafts, conflicts, blocking labels, unresolved reviews, stale approvals, missing/non-successful exact-head checks, and zero-step checks. `REQUIRED_CHECKS` exports `verify`, `schema-drift`, `report-validation`, `protected-data-boundary`; quality-gates is added when core/eval src changes. `CI_OUTAGE_FLAG` skips hosted checks when `reports/orchestrator/CI_OUTAGE` exists. 14 tests in `scripts/pi-merge-policy.test.mjs`. `scripts/pi-merge-loop.sh` uses `--match-head-commit` for all merges. (PR #188, commits 88017f8, a49fe3f)
...
| Merge control | Prototype | `scripts/pi-merge-policy.mjs` with fail-closed exact-head policy — gates on drafts, conflicts, blocking labels, unresolved reviews, stale approvals, missing/zero-step checks. `REQUIRED_CHECKS` exports `verify`, `schema-drift`, `report-validation`, `protected-data-boundary`. `CI_OUTAGE_FLAG` skips hosted checks when present. 14 tests in `scripts/pi-merge-policy.test.mjs`. `scripts/pi-merge-loop.sh` uses `--match-head-commit`. (PR #188, commits 88017f8, a49fe3f) |
