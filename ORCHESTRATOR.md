...
## Current State (2026-07-19, 23:43 EEST)
...
- **Automation hardening in this branch**: merge policy always requires exact-head checks, the committed `CI_OUTAGE` bypass is removed, and the merge loop no longer converts drafts to ready. The reviewer skips drafts and can issue only advisory comments—not ready labels—for CI, agent infrastructure, protected data, core/eval source, schemas, and registry changes. Tests prove callers cannot request a check skip.
- **PR #220**: draft exact head `a19ee3521ebfd97de9b2f41e934f2ede282702d5`, synchronized with `main`, labeled `needs-work`, `maintainer-blocked`, and `claude-review`. Earlier automated READY comments are invalid. Local `pnpm verify`, `pnpm verify:strict`, and `agent:status` pass, but three independent exact-head reviews returned `NEEDS_WORK`; local green checks do not satisfy the four release gates.
- **Exact-head blockers**: migration uses partial hand validation, accepts schema-invalid sources, emits schema-invalid 0.2 wrappers, and changes source file mode during atomic replacement. The protected quality runner also accepts schema-invalid/fingerprint-invalid records. Renderer goldens snapshot pass-through records rather than actual safe/short/tight Lunum-Code; safe/short are identical and tight usually only removes existing renderings. Token artifacts trust supplied counts, do not bind exact rendered bytes/renderer/tokenizer evidence, can mark a 91% regression valid, and the public original optimizer still compares a fingerprint with itself. The generic tokenizer approximation must not be presented as exact llama/Qwen/Mistral evidence. Profile documentation contradicts current behavior.
- **Repair orders active**: isolated workers are repairing (1) authoritative source/destination schema plus fingerprint validation, quality input validation, and mode-preserving atomic writes; (2) deterministic safe/short/tight Lunum-Code with meaningful exact reviewable goldens and corrected docs; and (3) injected exact-tokenizer measurement bound to rendered-byte, renderer, dataset, and tokenizer identities with genuine-improvement gates. No worker is authorized to push, merge, undraft, or change GitHub state; the orchestrator will integrate only reviewed local commits.
...
- **Next safe sequence**: review and integrate the three isolated repairs into #220 in one coherent non-force push; rerun focused, full, strict, adversarial, and agent-status checks on the exact integrated head; repeat independent exact-head review; keep #220 draft; wait for hosted checks that execute real steps; only then clear blockers, accept queue gates, close superseded #196/#214, and merge with exact-head binding. Keep persistent worker/reviewer/merge automation paused meanwhile.
