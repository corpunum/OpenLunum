...
- **PR #220**: draft exact head `a19ee3521ebfd97de9b2f41e934f2ede282702d5`, synchronized with `main`, labeled `needs-work`, `maintainer-blocked`, and `claude-review`. Earlier automated READY comments are invalid. Parallel migration, tokenizer, quality-gate, reviewable-golden, and renderer-conformance repairs were consolidated in one non-force push. Local `pnpm verify` and `agent:status` pass on the exact head.
- **Quality evidence**: the fail-closed runner now reports 100%, exit 0; renderer conformance is 10/10. Profiles preserve annotations, provenance, modality, time, and full role values, and loss-config flags are rejected. Human-readable exact outputs accompany hashes. Fresh independent subsystem reviews are running; keep all blocking labels and draft state until they finish.
...
- **Next safe sequence**: finish independent exact-head reviews; address any findings in one coherent push; keep #220 draft; wait for exact-head hosted checks with real steps; only then clear blockers, accept queue gates, close superseded #196/#214, and merge with exact-head binding. Keep persistent worker/reviewer/merge automation paused meanwhile.
