...
const report = { type: 'development-unimorph-unbiased-grounding', version: 1, sampleSize, languages: {} };
for (const [language, [repository, omwFile]] of Object.entries(files)) {
  ...
  for (const line of readFileSync(`${uniRoot}/${repository}/${repository}`, 'utf8').split(/\r?\n/u)) {
    ...
    const entries = forms.get(surface) ?? [];
    if (!entries.some((entry) => entry.lemma === lemma && entry.partOfSpeech === partOfSpeech)) entries.push({ lemma, partOfSpeech, evidence: [`unimorph:${repository}`] });
    forms.set(surface, entries);
  }
  const analyzer = { analyzer: 'unimorph', analyzerVersion: repository, snapshotHash: 'a'.repeat(64), analyze: ({ surface, partOfSpeech }) => (forms.get(surface) ?? []).filter((entry) => !partOfSpeech || entry.partOfSpeech === partOfSpeech) };
  const augmented = createMorphologyAugmentedProvider({ base, analyzer });
  const selected = [...forms.entries()].sort(([left], [right]) => createHash('sha256').update(left).digest('hex').localeCompare(createHash('sha256').update(right).digest('hex'), 'en')).slice(0, sampleSize);
  const counts = { total: selected.length, goldUnique: 0, rawIdentityAvailable: 0, rawCorrect: 0, candidateIdentityAvailable: 0, candidateCorrect: 0, candidateSetContainsGold: 0, ambiguous: 0, unresolved: 0, falsePositiveIdentity: 0 };
  for (const [surface, entries] of selected) {
    const goldIds = new Set();
    for (const entry of entries) {
      const gold = base.resolve({ proposal: proposal(entry.lemma), language, partOfSpeech: entry.partOfSpeech });
      for (const candidate of gold.candidates) goldIds.add(candidate.externalId);
    }
    const raw = base.resolve({ proposal: proposal(surface), language });
    const candidate = augmented.resolve({ proposal: proposal(surface), language });
  ...
  report.languages[language] = { repository, surfaceInventory: forms.size, counts, rates: { rawIdentityCoverage: counts.total ? counts.rawIdentityAvailable / counts.total : 0, rawExactAccuracyAll: counts.total ? counts.rawCorrect / counts.total : 0, candidateIdentityCoverage: counts.total ? counts.candidateIdentityAvailable / counts.total : 0, candidateExactAccuracyAll: counts.total ? counts.candidateCorrect / counts.total : 0, candidateSetRecallAll: counts.total ? counts.candidateSetContainsGold / counts.total : 0, ambiguityRate: counts.total ? counts.ambiguous / counts.total : 0, unresolvedRate: counts.total ? counts.unresolved / counts.total : 0 } };
}
