...
function rowFor(groupIndex, split, language, frame, outcome = 'parse') {
  ...
  return { id: `${group}-${language}`, split, source: { text, language, semanticGroup: group, templateFamily: `pilot-${predicate}-${String(groupIndex).padStart(3, '0')}`, conceptIds: [concept], entityIds: [`actor-${String(groupIndex).padStart(3, '0')}`] }, target: outcome === 'parse' ? { outcome, ir, ...(pair ? { criticalNegativePairIds: [pair.pairId] } : {}) } : { outcome: 'abstain', abstentionReason: abstainReasons[groupIndex % abstainReasons.length] }, provenance: { sourceKind: 'synthetic', annotationMethod: 'deterministic-template-v1', license: 'CC0-1.0', createdAt: '2026-09-06T00:00:00Z', generatorVersion: 'training-pilot/0.1' }, review: { status: 'accepted', reviewers: ['deterministic-template-validator'] } };
}
