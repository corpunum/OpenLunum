...
// Resolve to the actual shared checkout (not the worktree)
// From /home/corpunum/OpenLunum/.claude/worktrees/agent-xxx/scripts
// Go to /home/corpunum/OpenLunum
const WORKSPACE_ROOT = path.resolve(__dirname, '../../../..');

// Import the functions from the built packages
const { canonicalizeSem, stableStringify } = await import(path.join(
  WORKSPACE_ROOT,
  'packages/core/dist/src/canonicalize.js'
));

const { fingerprintSem } = await import(path.join(
  WORKSPACE_ROOT,
  'packages/core/dist/src/fingerprint.js'
));
