...
- Merge control: `scripts/pi-merge-policy.mjs` implements fail-closed exact-head policy — gates on drafts, conflicts, blocking labels, unresolved reviews, stale approvals, missing/non-successful exact-head checks, zero-step checks. `REQUIRED_CHECKS` exports `verify`, `schema-drift`, `report-validation`, `protected-data-boundary`; adds `quality-gates` when core/eval src changes. `CI_OUTAGE_FLAG` skips hosted checks when `reports/orchestrator/CI_OUTAGE` exists. 14 tests. (PR #188)
